...
from update import average_weights, average_gradients, local_trainer, noise_add
...
# user-level dp calculation
def privacy_account(args, q_s, local_data_size, threshold_epochs):
    delta_s = 2*args.clipthr/local_data_size
...
if __name__ == '__main__':
    ...
    parser.add_argument('--clipthr', default=0.25)
    ...
    for fold_idx in range(5):
        ...
        for epoch in range(int(args.num_epochs)):
            ...
            chosen_speakers = np.random.choice(range(num_of_speakers), int(0.1 * num_of_speakers), replace=False)
            
            # define list varibles that saves the weights, loss, num_sample, etc.
            local_updates, local_losses, local_num_sampels = [], [], []
            gradient_hist_dict = {}
            torch.save(deepcopy(global_model.state_dict()), str(model_result_path.joinpath('global_hist_'+str(epoch)+'.pt')))
            
            # 1. Local training, return weights in fed_avg, return gradients in fed_sgd
            for idx in chosen_speakers:
                ...
                if args.model_type == 'fed_avg':
                    local_update, train_result = trainer.update_weights(model=copy.deepcopy(global_model), clip=args.clipthr)
                else:
                ...
                noise_scale = privacy_account(args, 0.1, train_result['num_samples'], int(args.num_epochs))
                print('speaker id %s sample size %d noise scale %.4f' % (speaker_id, train_result['num_samples'], noise_scale))
                
                tmp_updates = copy.deepcopy(local_update)
                local_update = noise_add(noise_scale, local_update, device)
                ...
                gradient_hist_dict[speaker_id]['noise_scale'] = noise_scale
