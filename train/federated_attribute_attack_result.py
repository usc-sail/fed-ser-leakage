...
def run_one_epoch(data_loader):
    ...
    for batch_idx, data_batch in enumerate(data_loader):
        ...
        predictions = np.argmax(final_logits.detach().cpu().numpy(), axis=1)
        ...
        step_outputs.append({'loss': 0, 'pred': pred_list, 'truth': truth_list})

        del data_batch
        torch.cuda.empty_cache()
    result_dict = result_summary(step_outputs, mode='test', epoch=0)
    return result_dict
...
if __name__ == '__main__':
    ...
    if args.leak_layer == 'second' or args.leak_layer == 'fusion':
        ...
        attack_model_path = Path(os.path.realpath(__file__)).parents[1].joinpath('results', 'attack', 'second', args.model_type, args.feature_type, model_setting_str, 'private_' + str(args.dataset) + '.pt')
    ...
    if args.leak_layer == 'last' or args.leak_layer == 'fusion':
        ...
        attack_model_path = Path(os.path.realpath(__file__)).parents[1].joinpath('results', 'attack', 'last', args.model_type, args.feature_type, model_setting_str, 'private_' + str(args.dataset) + '.pt')
    ...
    for fold_idx in range(0, 5):
        ...
        test_result = run_one_epoch(test_loader)
    ...
    attack_model_result_csv_path = Path(os.path.realpath(__file__)).parents[1].joinpath('results', 'attack', args.leak_layer, args.model_type, args.feature_type, model_setting_str)
