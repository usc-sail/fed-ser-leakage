...
if __name__ == '__main__':
    ...
    parser.add_argument('--attack_dropout', default=0.1)
    ...
    attack_dropout_str = '_attack_dropout_' + str(args.attack_dropout).replace('.', '')
    ...
    for fold_idx in range(0, 5):
        ...
        for speaker_id in test_data_dict:
            ...
            attack_success_rate = np.mean(np.array(predictions_per_speaker) == np.array(truths_per_speaker))
        ...
        row_df['attack_success_rate'] = np.mean(attack_success_rate_list)
        save_result_df = pd.concat([save_result_df, row_df])
        
    row_df = pd.DataFrame(index=['average'])
    row_df['acc'], row_df['uar'] = np.mean(save_result_df['acc']), np.mean(save_result_df['uar'])
    row_df['attack_success_rate'] = np.mean(save_result_df['attack_success_rate'])
    save_result_df = pd.concat([save_result_df, row_df])
    
    model_setting_str += '_attack_dropout_' + str(args.attack_dropout).replace('.', '')
